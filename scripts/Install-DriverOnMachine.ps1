param ([string]$PkgName = "gamepad_info", [string]$CargoTargetDir = "target\debug", [string]$PkgTargetDestDir = "C:\test", [bool]$CleanOldVersion = $true, [bool]$CleanOldDriverOnTargetMachine = $true, [string]$VMName = "Windows 11 WDK Environment", [string]$CredentialPath = "$env:USERPROFILE\.cert\wdkcert.xml", [switch]$UninstallOnly)

$PkgName = "gamepad_info"
